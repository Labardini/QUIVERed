...
class planeGeometry:
    ...
    def coordsR2ToC(self, listWithTwoRealNumbers):
        """the input is an ordered list of two real numbers"""
...
class quiver:
    ...
    def CurveFormthArrowAsAFunction(self, initPt, finalPt, m):
        """the inputs are pairs (length-2 lists) of real numbers, and a non-negative integer"""
    ...
    def CurveFormthArrow(self, initPt, finalPt, m):
        """the inputs are pairs (length-2 lists) of real numbers, and a non-negative integer"""
    ...
    def TipFormthArrow(self, initPt, finalPt, m):
        """the inputs are pairs (length-2 lists) of real numbers, and a non-negative integers"""
        z0, z1 = self.Plane.coordsR2ToC(initPt), self.Plane.coordsR2ToC(finalPt)
        evaluationPointForTip = 0.525 * np.pi  # in the interval 0-pi
        if m == 0:
            position = [(z0 + evaluationPointForTip * (z1 - z0) / np.pi).real,
                        (z0 + evaluationPointForTip * (z1 - z0) / np.pi).imag]
            angle = 180 - self.Plane.angleRadiansToDegrees(z1 - z0)
        elif m > 0:
            position = [self.CurveFormthArrowAsAFunction(initPt, finalPt, m)(evaluationPointForTip).real,
                        self.CurveFormthArrowAsAFunction(initPt, finalPt, m)(evaluationPointForTip).imag]
            halfdifference = (z0 - z1) / 2
            adHocParam = 1 / 3
        ...
        return {"pos": position, "angle": angle}
